...
def control_pump(state):
    ...
    if response.status_code == 200:
        ...
        logging.info(f"Pump control successful, response: {response.text}")
    else:
        logging.info(f"Pump control failed, response code: {response.status_code}")
...
#############################################################################################     
    ...
    for ip_address, corners in garden_corners.items():
        ...
        if sensor_data['light_level'] < SUNNY and num_dry_pixels > MIN_DRY: #and wet_soil_ratio < WETDRY :      
            ...
            logging.info("Turning on the pump...")
            ...
            logging.info("Turning off the pump...")
        else:
